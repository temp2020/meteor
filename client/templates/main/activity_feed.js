...
angular
 ...
 function FeedsCtrl ($scope, $reactive, $state, $sce, $ionicLoading, $timeout, $translate, $ionicHistory) {
    ...
    this.go = function(id) {
      ...
      if ( thisFeed.linkType === 'product' ) {
        if( Listings.findOne({ _id: thisFeed.linkID, active: true }) ) {
          ...
          $translate('MESSAGE.TOAST.NOT_ACTIVE').then(function (message) {
            if (Meteor.isCordova) {
